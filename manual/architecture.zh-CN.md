...
它默认是「**本机单用户**」形态：`npx @deepseek-ai/dsh web` 在 `127.0.0.1:3080` 起一个 Web UI，插件与技能都挂在同一个 profile 下。
...
DSH 官方文档写的是「**你自己在本机跑的单用户安装**」。下面逐条点名：托管之后**哪些官方说法不再成立**，以及平台各自用什么替代。
...
官方明确说 DSH 处于 developer preview 且会有破坏性变更，并把安全提示指向 `SAFETY.md` —— 但它描述的是**单用户本机安装**，因此**不提供任何租户间隔离**。
