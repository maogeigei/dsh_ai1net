...
**No question is handed over bare.** Whatever the AI has to put to the human arrives with **two or three concrete options**, each carrying **what it is good for and what it costs** — that is the form **every** question takes, not a courtesy reserved for the hard ones. Judging those options is the human's part of the work — take one, put forward a different one, or ask for more before deciding. Where every option is one-sided there is nothing to judge, and it is settled without asking.

**Red lines are the questions that stand alone.** They must not be folded into something else: does this widen what can be reached, interrupt people who are using it right now, change more than a handful of files at once, or is it hard to undo. They arrive in the same form as everything else — options, with what each costs.
...
Two disciplines keep those layers from rotting. **The index carries pointers, not detail** — a fact written in two places is a fact that will eventually disagree with itself, so it lives in exactly one place and everything else links to it. And **no count is written into a document**: numbers like "how many documents" go stale faster than they can be read — several sessions edit the set in parallel, and on one day it went 83, then 85, then 87, which means writing it down once is writing it down wrong twice. So a document says where the number comes from and **the number is produced by running the command**, never copied out of the previous run.
