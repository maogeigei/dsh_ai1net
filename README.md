...
- **`manual/project.md` gains the collaboration model** — the decision method the human supplies, what that method lets the AI settle without asking again, where the method runs out, and how a red line is put to the human: as **two or three options with what each is good for and what it costs**, for them to judge. It also covers **how a document is found** in a set this size (a one-page brief, a scenario index, a generated manifest — with the index carrying pointers rather than detail, and no count written down by hand), the skills that fix the recurring procedures, and how work passes between sessions. Its section on working across context limits is retitled: the constraint is the size of the project, not memory.
