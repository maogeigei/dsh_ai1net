...
[![Version](https://img.shields.io/badge/version-v1.4.0-informational.svg)](#版本更新说明)
...
- **隔离是内核边界，不是约定** —— 一人一个 OS 账号、一套实例，叠加端口守卫与出网护栏。
- **没有节点开入站端口** —— 每个节点都只拨出；中继只绑回环，因此加节点永远不加公网可达面。
- **身份先于地址** —— 能不能入网由节点自己在本地判定，控制面被攻破也塞不进节点。
- **失败必具名** —— 被拒的入网、判死的直连、被拒的地址下发都带结构化原因；没有静默降级。
