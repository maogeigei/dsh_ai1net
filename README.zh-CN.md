...
- **`manual/project.md` 补上协作方式** —— 人给出的判断方法、方法覆盖得到哪些事可以让 AI 直接定、方法在哪里就到头了，以及红线怎么交给人判断：**两三个候选 + 每个的优点与缺点**。另含**在这么大的文档集里怎么找到该读的那一份**（一页现状卡 · 场景索引 · 自动生成的清单 —— 索引只放指针不放细节、且不写死任何计数）、把反复流程固化下来的技能，以及任务怎么在会话之间传递。该节标题也一并改为：约束是项目规模，不是记忆。
